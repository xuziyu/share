...
<body>
<a href="${ctxPath}/SysRoleAction.action?method=edit">新增</a>
<form action="${ctxPath}/SysRoleAction.action" id="form" method="post">
	<input type="hidden" id="pageIndex" name="pageIndex" value=""/>
	模块id：<input type="text" name="roleId"/>
	...
	<table>
		...
		<tbody>
			<c:forEach items="${page.result}" var="sysRole">
			<tr>
				<td>${sysRole.roleId}</td>
				<td>${sysRole.name}</td>
				<td>
					<a href="${ctxPath}/SysRoleAction.action?method=edit&id=${sysRole.roleId}" >修改</a>
					<a href="${ctxPath}/SysRoleAction.action?method=delete&id=${sysRole.roleId}" >删除</a>
				</td>
			</tr>
			</c:forEach>
		</tbody>
