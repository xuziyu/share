...
public class SalOrderDetailReviewDaoFront extends BaseDao implements ISalOrderDetailReviewDaoFront{
...
//			prepareStatement.setObject(5, salOrderDetailReview.getCreateDate());
	...
	public List<SalOrderDetailReview> load(String productSkuId) {
		PreparedStatement prepareStatement = null;
		ResultSet resultSet = null;
		List<SalOrderDetailReview> resultList=new ArrayList<SalOrderDetailReview>();
		try {
			StringBuffer sql =new StringBuffer("select sal_order_detail_review.* from sal_order_detail_review ") ;
			sql.append(" JOIN sal_order_detail on sal_order_detail_review.order_detail_id=sal_order_detail.order_detail_id");
			sql.append(" where product_sku_id = ");
			sql.append("'"+productSkuId+"'");
			System.out.println("load sql:" + sql);
			prepareStatement = DbConnection.getConnection().prepareStatement(sql.toString());
			//prepareStatement.setObject(1,productSkuId);
//			prepareStatement.setObject(1,"'"+productSkuId+"'");
			System.out.println("过了setObject");
			...
			if (resultSet.next()) {
				...
				salOrderDetailReview.setCreateDate(DateUtility.getUtilDate(resultSet.getTimestamp("create_date")));
				resultList.add(salOrderDetailReview);
			} else {
				System.out.println("SKU评论数据不存在");
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			throw new DbException(ex.getMessage(), ex);
	...
	public List<SalOrderDetailReview> pageResult(int pageIndex, int pageSize, String orderDetailReviewId,
		...
		if (!StringUtil.isEmpty(orderDetailId)) {
			sql.append(" and sal_order_detail.order_detail_id = ?");
